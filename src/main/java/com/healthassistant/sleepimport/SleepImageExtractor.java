...
class SleepImageExtractor {
    ...
    ExtractedSleepData extract(MultipartFile image, int year) throws SleepExtractionException {
        try {
            ...
            AiSleepExtractionResponse response = parseAiResponse(jsonContent);
    ...
    private AiSleepExtractionResponse parseAiResponse(String jsonContent) throws SleepExtractionException {
        try {
            return objectMapper.readValue(jsonContent, AiSleepExtractionResponse.class);
        } catch (Exception e) {
    ...
    private String validateNumericRanges(
        ...
        return Stream.of(
                        validateRange(totalSleepMinutes, "totalSleepMinutes", MAX_MINUTES_IN_DAY),
                        validateRange(sleepScore, "sleepScore", MAX_SLEEP_SCORE),
                        validateRange(lightSleepMinutes, "lightSleepMinutes", MAX_MINUTES_IN_DAY),
                        validateRange(deepSleepMinutes, "deepSleepMinutes", MAX_MINUTES_IN_DAY),
                        validateRange(remSleepMinutes, "remSleepMinutes", MAX_MINUTES_IN_DAY),
                        validateRange(awakeMinutes, "awakeMinutes", MAX_MINUTES_IN_DAY)
                )
    ...
    private Optional<String> validateRange(Integer value, String fieldName, int maxValue) {
        if (value != null && (value < 0 || value > maxValue)) {
            return Optional.of(String.format("Invalid %s: %d (must be 0-%d)", fieldName, value, maxValue));
        }
