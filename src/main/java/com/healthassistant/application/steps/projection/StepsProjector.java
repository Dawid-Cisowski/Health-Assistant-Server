package com.healthassistant.application.steps.projection;
...
public class StepsProjector {
    ...
    @Transactional
    public void projectSteps(Event event) {
        String eventType = event.eventType().value();

        if (STEPS_BUCKETED_V1.equals(eventType)) {
            try {
                projectStepsBucketed(event);
            } catch (org.springframework.dao.DataIntegrityViolationException e) {
                log.warn("Race condition during steps projection for event {}, skipping", event.eventId().value());
            }
        }
    }

    private void projectStepsBucketed(Event event) {
